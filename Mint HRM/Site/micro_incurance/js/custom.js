...
//      Transport radio buttn
	$('.transport').lc_switch();
        

	// triggered each time a field changes status
	$('body').delegate('.transport', 'lcs-statuschange', function() {
		var status = ($(this).is(':checked')) ? 'checked' : 'unchecked';
		console.log('field changed status: '+ status );
	});
	
	// triggered each time a field is checked
	$('body').delegate('.transport', 'lcs-on', function() {
                 $(this).val('1');
                  var id = $(this).attr('data-tab');
                  $('.dtl-tab').slideUp();
                  $('#' + id).slideDown();
	...
	// triggered each time a is unchecked
	$('body').delegate('.transport', 'lcs-off', function() {
                 $(this).val('0');
	});
        
        
//      Hotel Rates radio buttn
	$('.hotel-rates').lc_switch();
        

	// triggered each time a field changes status
	$('body').delegate('.hotel-rates', 'lcs-statuschange', function() {
		var status = ($(this).is(':checked')) ? 'checked' : 'unchecked';
		console.log('field changed status: '+ status );
	});
	
	// triggered each time a field is checked
	$('body').delegate('.hotel-rates', 'lcs-on', function() {
                 $(this).val('1');
                 $('.flight-details').slideDown();
                
                // $('#'+id).css('display','block')
	});
	
	// triggered each time a is unchecked
	$('body').delegate('.hotel-rates', 'lcs-off', function() {
                 $(this).val('0');
	});

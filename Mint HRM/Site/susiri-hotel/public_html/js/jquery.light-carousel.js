		...
		this.each(function(){
			...
			window.onload = function() {
				...
				if((typeof window.orientation != 'undefined') || (navigator.userAgent.indexOf('Windows Phone') != -1)){
					$('.carousel-1').addClass('mobile');
				} // Hide controls for mobile

				$(container).fadeIn(1000);
				setTimeout(function(){
					$('.carousel-1 li:first').addClass('current');
				}, 200);

				$('.thumbnails li:first').addClass('selected');
				$('.carousel-1').height($('.carousel-1 li:first').outerHeight() - 1);
				...
				for(var i = 0; i < $('.carousel-1 li').length; i++) {
					var caption = $('.carousel-1 li').eq(i).children('img').attr('alt');
					$('.carousel-1 li').eq(i).append('<div class="caption"><span>' + caption + '</span></div>');
					slideLength++;
				...
				function changeSlide(number) {
					if(options.changeHeight){
						$('.carousel-1').height($('.carousel-1 li').eq(number).outerHeight() - 1);
					}
					$('.carousel-1 li').removeClass('current');
					$('.carousel-1 li').eq(number).addClass('current');
				...
				$(window).on("resize", function() {
					...
					timer = setTimeout(function() {
						$('.carousel-1').height($('.carousel-1 li').eq(selectedNumber).outerHeight() - 1);
					}, 50);
				});
			}
		});
